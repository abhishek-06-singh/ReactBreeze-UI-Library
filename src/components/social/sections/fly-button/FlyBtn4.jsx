    ...
    <div className="flex items-center justify-center h-96 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-100 via-indigo-300 to-pink-300">
      <div>
        <div className={"absolute bottom-0 right-0 mb-6 mr-6 group"}>
          ...
          {isContactVisible && (
            <div className={`absolute bottom-16 right-0 space-y-4 mb-10`}>
              {contacts.map((contact) => (
                <div
                  key={contact.id}
                  className={` transition-all duration-500 ease-in-out flex items-center justify-start bg-slate-800 rounded-xl w-72 h-28 hover:bg-gray-950 p-4 ${
                ...
                >
                  ...
                  <div className="ml-10 flex flex-col">
                    <div className="flex items-center mb-2">
                      ...
                      ></span>
                      <p className="text-slate-50 text-xs">{contact.status}</p>
                    </div>
                    <p className="text-slate-50 font-bold">{contact.name}</p>
                    <p className="text-slate-50 text-sm">{contact.phone}</p>
