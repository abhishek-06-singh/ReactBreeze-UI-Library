...
const ProductCardDark = () => {
  const [modalIsOpenDark, setModalIsOpenDark] = useState(false);
  ...
  const openModalDark = () => {
    setModalIsOpenDark(true);
  };

  const closeModalDark = () => {
    setModalIsOpenDark(false);
  };
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-100 via-indigo-300 to-pink-300">
      <div className="max-w-sm bg-slate-800 border border-slate-700 rounded-lg shadow">
        <a href="#">
        ...
        {modalIsOpenDark && (
          <div className="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center bg-black bg-opacity-10">
            <div className="relative max-w-lg mx-auto p-4 bg-slate-900 rounded-xl shadow-lg">
              <div
                className="absolute top-1 right-1 cursor-pointer bg-indigo-600 justify-center items-center flex-shrink-0 text-gray-200 hover:text-gray-300 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-indigo-500 inline-flex h-8 w-8 z-40"
                onClick={closeModalDark}
              >
                <CrossSvgIconWhite />
              </div>
              <img
                ...
                alt=""
              />
              <h5 className="mb-4 text-2xl font-bold tracking-tight text-gray-50">
                {product.title}
              </h5>
              ...
              <p className="mb-4 text-gray-100  overflow-y-auto h-40 scrollbar-hide text-sm">
                {product.fulldescription}
              </p>
              <div className="text-green-500 text-md font-medium">{`$${product.price}`}</div>
              <div className="text-gray-100">{product.available}</div>
            </div>
          </div>
        )}
      </div>
