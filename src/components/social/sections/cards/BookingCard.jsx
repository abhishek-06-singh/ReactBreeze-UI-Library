...
const BookingCard = () => {
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-100 via-indigo-300 to-pink-300">
      <div className="mx-auto mt-8 bg-white rounded-2xl overflow-auto scrollbar-hide shadow-lg w-4/12 h-5/6 ">
        <div className="relative">
          ...
          />
          <button className="absolute top-0 right-0 bg-gray-50 text-gray-700 hover:bg-gray-200 hover:text-gray-800 px-1.5 py-1.5 rounded-2xl m-1">
            <CrossSvgIconWhite />
          </button>
        </div>
        <div className="p-4 h-1/2 pl-7 pr-7 ">
          ...
          <p className="text-gray-700 text-sm h-10 overflow-auto scrollbar-hide ">
            {product.description}
          </p>
          <hr className="mt-2" />
          <div className="mt-2">
            <h3 className="font-bold mb-2">Service Includes:</h3>
            <ul className="list-none flex">
              {renderServiceIncludes().map((item, index) => (
                <li
                  key={index}
                  className="bg-indigo-200 text-gray-800 rounded-md ml-2 p-1 hover:scale-110 transition-all ease-in-out duration-300 cursor-pointer mb-6"
                >
                  {item}
                </li>
              ))}
          ...
          <DatePicker
            ...
            className="mt-1 bg-indigo-50 w-96 text-center text-indigo-800 font-semibold border-indigo-900 border-opacity-50 border border-solid rounded-3xl bg-opacity-75 py-3"
          />
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900 bg-opacity-50 flex items-center justify-center">
          <div className="bg-slate-50 p-4 rounded-lg w-56 relative transform ease-in-out duration-300">
            <h3 className="font-bold mb-2 text-gray-900">Service Includes:</h3>
            <button
            ...
            <ul className="items-center">
              {product.serviceIncluded.map((item, index) => (
                <li key={index} className="mb-2 text-slate-950">
