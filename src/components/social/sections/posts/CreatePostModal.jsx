...
const CreatePostComponent = ({}) => {
  ...
  return (
    <div className="flex items-center justify-center h-96 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-slate-100 via-indigo-300 to-pink-300">
      <div className="bg-black p-4 rounded-2xl max-h-3/4 w-5/12 relative overflow-hidden overflow-y-hidden scrollbar-hide">
        {uploadedFiles.length > 2 && (
          <>
            <button
              onClick={scrollLeft}
              className="text-white absolute left-10 transform -translate-x-1/2 top-1/2 -translate-y-1/2"
            >
              <FaArrowLeft />
            </button>
            <button
              onClick={scrollRight}
              className="text-white absolute right-5 transform translate-x-1/2 top-1/2 -translate-y-1/2"
            >
              <FaArrowRight />
            </button>
          </>
        ...
        <div className="text-white pl-8 pr-4">
          <input
            type="text"
            placeholder="Start the thread..."
            value={postContent}
            onChange={(e) => setPostContent(e.target.value)}
            className="border-0 p-2 outline-none bg-black text-white w-full"
          />
              ...
              {uploadedFiles.map((file) => (
                <div key={file.id} className="inline-block mr-2 relative">
                  <button
                    ...
                    className="absolute top-0 right-0 p-1 bg-gray-600 rounded-full text-white w-6 h-6 text-xs"
                  >
                    X
                  </button>
        ...
        <div className="flex items-center justify-between mt-6">
          ...
          <button
            onClick={handlePost}
            className="bg-gray-500 text-white px-2 py-2 rounded-xl w-40"
          >
            Post
          </button>
        </div>
